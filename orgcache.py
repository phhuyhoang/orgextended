...
from OrgExtended.orgutil.sublime_utils import set_temporary_status, show_message
    ...
    * ``"data.size"``
    ...
    def run(self, target: str = '') -> None:
        ...
        try:
            ...
            return set_temporary_status(self.window, STATUS_ID, str(error), 3)
        else:
            debug_info = dict()
            debug_info['max_size'] = sizeof_fmt(cache.current_max_size)
            debug_info['used_size'] = sizeof_fmt(cache.size())
            debug_info['data'] = []
